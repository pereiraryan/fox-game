...
const gameState = {
  ...
  die() {
    ...
    (0, _ui.modFox)('dead');
    this.clearTime();
    (0, _ui.writeModal)("The Fox died: Press the middle button to start again");
  },
...
function initButtons(handleUserAction) {
  ...
  function buttonClick({
    ...
    if (target.classList.contains("left-btn")) {
      ...
      selectedIcons = (2 + selectedIcons) % _constants.ICONS.length;
      toggleHighlighted(selectedIcons, true);
    } else if (target.classList.contains("right-btn")) {
      toggleHighlighted(selectedIcons, false);
      selectedIcons = (1 + selectedIcons) % _constants.ICONS.length;
      toggleHighlighted(selectedIcons, true);
    } else {
      handleUserAction(_constants.ICONS[selectedIcons]);
    }
  }
